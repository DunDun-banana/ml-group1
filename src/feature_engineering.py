...
def create_lag_rolling(df, columns, lags=(1,2,3,4,5,6,7), windows=(3, 7, 14), fill_method="bfill"):
    df = df.sort_index()
    new_features = []

    lag_df = pd.concat(
        {f"{col}_lag_{l}": df[col].shift(l) for col in columns for l in lags},
        axis=1
    )
    new_features.append(lag_df)
...
def feature_engineering(df, column, forecast_horizon=5):
    """
    Thực hiện toàn bộ quy trình Feature Engineering cho bài toán dự báo đa đầu ra.

    Outputs:
    - df: DataFrame đã được thêm các feature mới và có các cột target tương ứng.
    - target_cols: Danh sách tên các cột target được tạo ra.
    """
    # Sử dụng shift âm để "nhìn" vào tương lai
    target_cols = []
    for i in range(1, forecast_horizon + 1):
        target_col = f"temp_next_{i}"
        df[target_col] = df['temp'].shift(-i)
        target_cols.append(target_col)

    # Tạo các features dựa trên thông tin có sẵn tại ngày hiện tại (t)
    df = create_date_features(df)
    df = create_specific_features(df)

    # Tạo các features dựa trên thông tin quá khứ (t-1, t-2, ...)
    # Chúng ta có thể dùng lags từ 1 đến 7 ngày trước.
    df = create_lag_rolling(df, columns=column, lags=range(1, 8), windows=(3, 7, 14))

    # Loại bỏ các hàng có giá trị NaN.
    # Thao tác này sẽ loại bỏ:
    # 1. Các hàng cuối cùng nơi không có target (do shift âm).
    # 2. Các hàng đầu tiên nơi không có đủ dữ liệu cho lag/rolling.
    df = df.dropna()
